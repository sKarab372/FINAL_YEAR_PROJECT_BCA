...
const ML_API = process.env.REACT_APP_ML_URL || "http://127.0.0.1:5001/api";
...
const Ticker = ({ stocks }) => {
  if (!stocks.length) return (
    <div style={{ height:28, borderBottom:"1px solid #1A2235", background:"#070A0F" }} />
  );
      ...
      <div style={{ display:"flex", animation:"ticker 40s linear infinite", whiteSpace:"nowrap" }}>
        {doubled.map((s, i) => (
          <span key={i} style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"0 24px", fontFamily:"var(--mono)", fontSize:11, lineHeight:"28px" }}>
            ...
            <span style={{ color: s.pct >= 0 ? "#26D97F" : "#FF4D6A" }}>
...
function AIChat({ selectedStock }) {
  const [messages, setMessages] = useState([
    { role:"assistant", content:`Terminal online. Analyzing **${selectedStock.symbol}** — ${selectedStock.name}. Ask me anything about this stock, market conditions, or trading strategy.` }
  ]);
  ...
  useEffect(() => {
    setMessages([{ role:"assistant", content:`Terminal online. Analyzing **${selectedStock.symbol}** — ${selectedStock.name}. Ask me anything about this stock, market conditions, or trading strategy.` }]);
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      const systemPrompt = `You are a sharp, concise financial analyst AI embedded in a stock trading terminal. You're currently analyzing ${selectedStock.symbol} (${selectedStock.name}) trading at $${selectedStock.price} (${selectedStock.pct >= 0 ? "+" : ""}${selectedStock.pct}% today). Sector: ${selectedStock.sector}. Market cap: ${selectedStock.mktCap}. P/E: ${selectedStock.pe}. Keep responses brief (2-4 sentences), data-driven, and direct.`;
      ...
      setMessages(m => [...m, { role:"assistant", content:"Connection error. Check your network and API key." }]);
  ...
  const renderContent = (text) =>
    text.split(/(\*\*[^*]+\*\*)/).map((part, i) =>
      part.startsWith("**")
        ? <strong key={i} style={{ color:"#F0C040" }}>{part.slice(2, -2)}</strong>
        : part
    );

  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", fontFamily:"var(--body)" }}>
      <div style={{ padding:"12px 16px", borderBottom:"1px solid #1A2235", display:"flex", alignItems:"center", gap:8 }}>
        <div style={{ width:6, height:6, borderRadius:"50%", background:"#26D97F", animation:"pulse-dot 2s ease infinite" }} />
        <span style={{ fontFamily:"var(--display)", fontWeight:700, fontSize:12, letterSpacing:"0.12em", color:"#7A8FAD", textTransform:"uppercase" }}>AI Analyst Terminal</span>
      </div>
      ...
      <div style={{ flex:1, overflow:"auto", padding:"12px 16px", display:"flex", flexDirection:"column", gap:10 }}>
        {messages.map((m, i) => (
          <div key={i} className="fade-in" style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
            ...
            <div style={{ maxWidth:"82%", padding:"8px 12px", borderRadius:m.role==="user"?"8px 8px 2px 8px":"2px 8px 8px 8px", background:m.role==="user"?"#0F1E38":"#0C1018", border:`1px solid ${m.role==="user"?"#243048":"#1A2235"}`, fontSize:13, lineHeight:1.6, color:m.role==="user"?"#CDD6E8":"#A8BBCC" }}>
        ...
        {loading && (
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:20, height:20, borderRadius:2, background:"#0F1A2E", border:"1px solid #243048", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <span style={{ fontFamily:"var(--mono)", fontSize:8, color:"#F0C040" }}>AI</span>
            </div>
            <div style={{ padding:"8px 14px", background:"#0C1018", border:"1px solid #1A2235", borderRadius:"2px 8px 8px 8px" }}>
              {[0,1,2].map(n => (
                <span key={n} style={{ fontFamily:"var(--mono)", fontSize:16, color:"#F0C040", animation:`pulse-dot 1.2s ease ${n*0.2}s infinite` }}>.</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{ padding:"12px 16px", borderTop:"1px solid #1A2235" }}>
        ...
        <div style={{ fontFamily:"var(--mono)", fontSize:10, color:"#2A3A52", marginTop:6 }}>
          Powered by Claude · {selectedStock.symbol} context loaded
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Fetch watchlist stocks (parallel on server side)
  useEffect(() => {
    ...
    fetch(`${API}/stocks?symbols=${watchlist.join(",")}`, { signal: controller.signal })
      ...
      .catch(err => {
        if (err.name !== "AbortError") {
          setStocksError("Could not connect to server. Is server.py running on port 5000?");
          setLoadingStocks(false);
        }
      });
  ...
  const forecastStartDate = showForecast && mlPrediction?.forecast?.length
    ? mlPrediction.forecast[0].date : null;
    ...
    <>
      ...
      <div style={{ display:"flex", flexDirection:"column", height:"100vh", overflow:"hidden", background:"var(--bg)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", padding:"0 20px", height:44, borderBottom:"1px solid #1A2235", background:"#070A0F", gap:16, flexShrink:0 }}>
          ...
          <div style={{ fontFamily:"var(--mono)", fontSize:10, color:"#4A5A72" }}>
            {new Date().toUTCString().slice(0, 25)} UTC
          </div>
          <div style={{ marginLeft:"auto", display:"flex", gap:6, alignItems:"center" }}>
            <div style={{
              width:6, height:6, borderRadius:"50%",
              background: stocksError ? "#FF4D6A" : loadingStocks ? "#F0C040" : "#26D97F",
        ...
        <div style={{ display:"grid", gridTemplateColumns:"210px 1fr 340px", flex:1, overflow:"hidden" }}>
          ...
          <div style={{ borderRight:"1px solid #1A2235", overflow:"auto", background:"#070A0F", display:"flex", flexDirection:"column" }}>
            ...
            {/* Error state */}
            {stocksError && (
              <div style={{ padding:16, fontFamily:"var(--mono)", fontSize:11, color:"#FF4D6A", lineHeight:1.6 }}>
                ⚠ {stocksError}
              </div>
            )}
              ...
              return (
                ...
                >
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:2 }}>
                    ...
                    <span style={{ fontFamily:"var(--mono)", fontSize:10, color:s.pct>=0?"#26D97F":"#FF4D6A" }}>
                      {s.pct>=0?"▲":"▼"}{Math.abs(s.pct).toFixed(2)}%
                    </span>
                  </div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <span
                      ...
                      style={{ fontFamily:"var(--mono)", fontSize:10, color:"#2A3A52", cursor:"pointer" }}
                      title="Remove"
                    >✕</span>
                  </div>
                </div>
          ...
          {loadingStocks && !stocksError ? (
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", flex:1, flexDirection:"column", gap:16 }}>
              ...
              <div style={{ fontFamily:"var(--mono)", fontSize:12, color:"#4A5A72", textAlign:"center", lineHeight:1.8 }}>
                ...
                Run: <span style={{ color:"#F0C040" }}>python server.py</span>
              </div>
            </div>
          ) : !stock ? (
            <Spinner />
            ...
            <div style={{ overflow:"auto", display:"flex", flexDirection:"column", background:"#070A0F" }}>
              ...
              <div style={{ display:"flex", gap:1, padding:"12px 24px", flexWrap:"wrap" }}>
                ...
                {mlPrediction && (
                  <>
                    ...
                    <StatBox
                      label="DL Target (14d)"
                      value={`$${mlPrediction.targetPrice}`}
                ...
                {mlLoading && (
                  <>
                    <div style={{ width:1 }} />
                    <StatBox label="DL Signal"       value="—" sub="Training model..." accent="#4A5A72" loading />
                    <div style={{ width:1 }} />
                    <StatBox label="DL Target (14d)" value="—" sub="30–90s on CPU"    accent="#4A5A72" loading />
                  </>
                )}
              </div>

              {/* time range + predict button */}
              <div style={{ display:"flex", gap:4, padding:"0 24px 12px", alignItems:"center", flexWrap:"wrap" }}>
                ...
                <button
                  ...
                  style={{
                    padding:"4px 16px",
                    background: mlLoading ? "#0C1018" : showForecast ? "#26D97F" : "#111820",
                    color: mlLoading ? "#4A5A72" : showForecast ? "#070A0F" : "#26D97F",
                    border:`1px solid ${mlLoading ? "#1A2235" : "#26D97F"}`,
                ...
                >
                  {mlLoading ? (
                    <>
                      <div style={{ width:8, height:8, border:"1px solid #4A5A72", borderTop:"1px solid #26D97F", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
                      TRAINING...
                    </>
                  ) : showForecast ? "✓ FORECAST ON" : "⚡ RUN TRANSFORMER"}
                </button>

                {showForecast && (
                  <button
                    onClick={() => { setShowForecast(false); setMlPrediction(null); }}
                    style={{ padding:"4px 10px", background:"transparent", color:"#4A5A72", border:"1px solid #1A2235", borderRadius:3, fontSize:11 }}
                  >
                    CLEAR
                  </button>
                )}
                ...
                {mlError && (
                  <span style={{ fontFamily:"var(--mono)", fontSize:10, color:"#FF4D6A", marginLeft:8 }}>
                    ⚠ {mlError}
                  </span>
                )}
              </div>
                ...
                {loadingHistory ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top:10, right:10, left:0, bottom:0 }}>
                      ...
                      <XAxis
                        dataKey="date"
                        tick={{ fontFamily:"'Share Tech Mono'", fontSize:9, fill:"#4A5A72" }}
                        tickLine={false}
                      ...
                      <Area
                        ...
                        fill="url(#goldGrad)"
                        dot={false}
                        activeDot={{ r:4, fill:"#F0C040", strokeWidth:0 }}
                        connectNulls={false}
                      />
                      {showForecast && (
                        <Area
                          ...
                          fill="url(#greenGrad)"
                          dot={false}
                          activeDot={{ r:4, fill:"#26D97F", strokeWidth:0 }}
                          connectNulls={false}
                        />
                      )}
                      {forecastStartDate && (
                        <ReferenceLine
                          ...
                          stroke="#243048"
                          strokeDasharray="3 3"
                          label={{ value:"Forecast →", position:"top", fill:"#26D97F", fontSize:9, fontFamily:"'Share Tech Mono'" }}
                        />
                      )}
                    </AreaChart>
                  </ResponsiveContainer>
              ...
              <div style={{ display:"flex", gap:20, padding:"0 24px 16px" }}>
                {[
                  ["#F0C040", "Price"],
                  ...(showForecast ? [["#26D97F", "Transformer Forecast (14d)"]] : [])
                ].map(([color, label]) => (
          ...
          {/* CHAT */}
          <div style={{ borderLeft:"1px solid #1A2235", display:"flex", flexDirection:"column", background:"#070A0F" }}>
            {stock
